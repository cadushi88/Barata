...
function StoresPage() {
  ...
  return (
    <Shell>
      ...
      {q.isError ? (
        <p className="mt-6 text-sm text-warn">Could not load stores. Refresh in a moment.</p>
      ) : q.isLoading ? (
        <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="h-24 animate-pulse rounded-md bg-line/60" />
          ))}
        </div>
      ) : (q.data ?? []).length === 0 ? (
        <div className="mt-6 rounded-md border border-line bg-surface px-4 py-8 text-center">
