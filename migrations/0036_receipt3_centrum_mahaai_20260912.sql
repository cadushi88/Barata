...
-- both check out exactly against this transcription: 2 Champignon + 5 Royal
-- Gelatin + 1 Cilantro + 2 Popcorn + 1 Squash + 1 Chicken Wings + 1 Ribeye +
-- 4 Canada Dry + 4 Tuna + 4 Alpro + 1 Knoflook = 26 lines, and summing every
-- line's printed unit price (6.25*2 + 2.83*5 + 4.25 + 8.51*2 + 8.81 + 8.98 +
-- 34.30 + 1.60*4 + 2.38*4 + 5.95*4 + 1.95) = XCG 141.68 exactly. The
-- "Markdown: Cg 1.22" printed under each Alpro line (4 * 1.22 = the printed
-- "Saving grand total" of Cg 4.88) is informational only -- it is not
-- subtracted from Net/Total Sales/Sub Total/Amount Charged, all four of
-- which equal the undiscounted 141.68 -- so the price recorded per Alpro
-- unit below is the printed 5.95, i.e. what was actually charged.
--
...
--   - Champignon Middel 250gr, Squash Zuchinni XFCY USA k, Frozen Ribeye
--     Barra Brasil, Canada Dry Ginger All Zero, Tuna PRF Catch Shredded in
--     Water, and Knoflook [Garlic] 250gr have no existing catalog match at
--     all -- all six are new products.
...
insert into products (slug, name, brand, category, unit, unit_size, unit_kind) values
  ...
  ('squash-zucchini-xfcy-p-kg', 'Squash Zucchini XFCY', null, 'Produce', 'p/kg', null, null),
...
insert into _receipt3_prices (slug, raw_name, raw_price) values
  ...
  ('squash-zucchini-xfcy-p-kg', 'Squash zuchinni xfcy usa k', 8.81),
